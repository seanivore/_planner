...
- Stack named precisely: React 18, TypeScript, Vite, Tailwind, shadcn/ui, Supabase (Postgres + RLS), Stripe Custom Checkout, Vercel, GitHub Actions, Cloudinary, Cloudflare R2.
- Senior-tell upgrade: opened the payments.august.style bullet with the problem definition ("Client needed a payment platform without DevOps overhead or recurring infrastructure cost..."). Everlastings bullet leads with the client-shape problem ("Non-technical owner needed to self-manage..."). PETA opener owns the problem too.
- PETA + Silent Labs web/product relevance is now front-loaded: Silent Labs leads with Webflow + Figma + front-end UI partnership; PETA leads with mobile app UI/UX and Optimizely landing-page A/B testing — both anchored to web/product surfaces, not generic marketing wins.
...
Web designer building end-to-end product experiences — React/TypeScript on the engineering side, type and motion craft on the design side. Currently shipping three live AI-augmented sites: a JSON-as-database payments platform, a Supabase-backed e-commerce store, and a 37-entry portfolio CMS with pre-rendered SEO.

## DESIGN, ENGINEERING & AI TOOLS

**Design:** Figma, Webflow, Framer, design systems, type and color tokens, CSS animations, Lottie, After Effects, WCAG accessibility, responsive layout, prototyping
**Engineering:** React 18, TypeScript, Vite, Tailwind, shadcn/ui, Supabase (Postgres + RLS), Stripe Custom Checkout, Vercel serverless, GitHub Actions, Cloudinary, Cloudflare R2 CDN, pdfjs-dist
**AI-Assisted Workflow:** Claude Code, Figma AI, custom MCP servers, agentic component generation
...
- Client needed a freelance payment platform without DevOps overhead or recurring infrastructure cost; architected payments.august.style on React 18 + TypeScript + Vite + Stripe Custom Checkout with JSON files as the source of truth and GitHub Actions as the back end — total infrastructure cost $0/month versus the $30–150/month a comparable Postgres + server stack would carry.
- Non-technical owner needed to self-manage products through a browser; shipped everlastingsbyemaline.com on Supabase + Vercel serverless + Stripe + Cloudflare R2 with a custom admin panel, tuned to ~$15–75/year operating cost.
...
- Collaborated with a game-development agency and two iPhone direct-action app teams on UI/UX — feature scoping, motion and interaction reviews, asset specs — for an app that reached 1M+ downloads and a 40% engagement lift from UX writing changes.
- Ran Optimizely A/B testing across landing pages and Facebook campaigns, driving CPA down to $0.002 across a 3.2B-impression Facebook year.
- Set production standards and trained Philippines, India, and UK affiliate teams on creative format, asset specs, and analytics review.

## EDUCATION

**MORAVIAN UNIVERSITY — Bethlehem, PA** — Bachelor of Arts, Cognitive Psychology; GPA 3.5 (2005–2009). Concentrations: Marketing & Advertising; Historic Darkroom Photography Processes.
